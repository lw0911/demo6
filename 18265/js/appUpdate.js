...
function appUpdate() {
    plus.runtime.getProperty(plus.runtime.appid, function (inf) {
        ver = inf.version;
        console.log('ver:' + ver);
        var url = rootUrl + 'Server/ConfigHandler.ashx?method=getVersion';
        var client;
            ...
            mui.ajax(url, {
                ...
                success: function (data) {
                    ...
                    if (data.StatusCode = 200 && data.data > ver) {
                        //mui.toast("发现新版本:V" + data);//获取远程数据库中上新andriod版本号 
                        var _msg = "发现新版本:V" + data.data;
                        mui.confirm(_msg, '升级确认', btn, function (e) {
                            if (e.index == 0) { //执行升级操作
                                plus.nativeUI.toast("正在准备环境，请稍后！");
                                var dtask = plus.downloader.createDownload("http://219.159.239.222:8089/slgtydzhpt/app.apk", {}, function (d, status) {
                                    if (status == 200) {
